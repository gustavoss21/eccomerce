...
from produto.views import Carrinho
...
class Pagar(View):
    ...
    def get(self, *args, **kwargs):
        if not self.request.user.is_authenticated:
            messages.error(
                self.request, 'você precisa fazer login'
            )
        ...
        for variacao in bd_variacoes:
            ...
            if estoque < qtd_carrinho:
                ...
                error_msg_estoque = 'Estoque insuficiente para alguns produtos do seu carrinho.'\
                    'e a quatidade foi ajustado de acordo com o estoque'

            if error_msg_estoque:
                messages.error(self.request, error_msg_estoque)
                self.request.session.save()
                return redirect('produto:carriho')

        qtd_total_carrinho = utils.cart_total_qtd(carrinho)
        valor_total_carrinho = utils.cart_totais(carrinho)
